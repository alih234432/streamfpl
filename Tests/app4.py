...
ENTRY_URL = f"{BASE_URL}entry/" # Added endpoint for team ID lookup
...
@st.cache_data(ttl=3600)
def get_user_team_data(team_id):
    """Fetch team data for a specific FPL team ID."""
    try:
        # First get the basic team info
        team_info = requests.get(f"{ENTRY_URL}{team_id}/").json()
        
        # Then get the current team picks
        current_gw = get_current_gameweek(get_fpl_data()['events'])
        team_picks = requests.get(f"{ENTRY_URL}{team_id}/event/{current_gw}/picks/").json()
...
def get_upcoming_fixtures(team_id, fixtures_data, teams_data, num_fixtures=5):
    ...
    for fixture in fixtures_data:
        ...
        if fixture['event'] and fixture['event'] >= current_gw:
            ...
            # Get team name for the opponent using the mapping
...
def get_assistant_response(prompt, fpl_data=None, fixtures_data=None, user_team=None):
    ...
    if fpl_data:
        ...
        # Format fixtures in a readable way
        fixtures_info = ""
        if fixtures_data:
            # Get upcoming fixtures for a few popular teams
            popular_teams = [1, 3, 6, 13]  # Arsenal, Chelsea, Liverpool, Man City
            for team_id in popular_teams:
                team_name = next((team['name'] for team in teams_data if team['id'] == team_id), f"Team {team_id}")
                upcoming = get_upcoming_fixtures(team_id, fixtures_data, teams_data, 3)
                if upcoming:
                    fixtures_info += f"\n{team_name} upcoming fixtures:\n"
                    for fixture in upcoming:
                        opponent = fixture['opponent']
                        location = "vs" if fixture['is_home'] else "@"
                        fixtures_info += f"GW{fixture['gameweek']}: {location} {opponent}\n"
        
        # Add context about the data
        context = f"""
        You are an FPL Assistant with access to the latest Fantasy Premier League data for the 2024/2025 season.
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a helpful Fantasy Premier League assistant for the 2024/2025 season. Always provide advice based on the current season data."},
    ...
    if team_input_option == "Team ID":
        ...
        if team_id and st.button("Load Team"):
            try:
                ...
                team_data = get_user_team_data(team_id)
                
                if team_data:
                    # Extract player IDs from the picks
                    player_ids = [pick['element'] for pick in team_data['picks']['picks']]
                    
                    # Save to session state
                    st.session_state.user_team = player_ids
                    st.success(f"Team loaded successfully! {len(player_ids)} players found.")
                else:
                    st.error("Could not load team data. Check your team ID.")
            except Exception as e:
